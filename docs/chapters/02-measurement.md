...
**教師ありファインチューニング**を行った構成が、必ず一番よいとは限りません。学習前の生徒モデルが必要な品質を満たすなら、追加の学習費を払わずに使う選択も評価の対象にします。
...
採点には、ツールや状態のルール検査と、日本語の説明などを読む採点用モデルを組み合わせます。利用者は自動採点後の比較表で代表例を確認します。自動判定は推定であり、記録だけでは判断できない点は保留します。人が確認した業務成功や最終的な採用判断とは分け、評価方法の説明と、実環境での検証が完了していることも区別します。
