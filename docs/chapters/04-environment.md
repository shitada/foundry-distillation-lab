...
| 決める項目 | 記録する内容 |
|---|---|
| 学習・評価への投資 | 期待する効果と追加学習の理由。データ生成・学習・評価の予算。小規模試行と本実験の上限を分ける |
...
| 実行の承認 | 対象・入力・件数・費用上限と通貨・期限・実行者に結び付けた承認 |
...
入力や業務仕様が途中で変わっていないかを確認するために、内容から計算した識別値である**ハッシュ値**も記録します。ファイルや設定が確定した段階で、実験計画と対応付けて保存します。
...
この教材では、操作ごとの承認内容をファイルへ記録し、実行プログラムが対象・入力・予算・期限を照合します。[承認ファイルの設定例](https://github.com/shitada/foundry-distillation-lab/blob/main/configs/examples/approval.json)は、項目を確認するためのものです。例の数値や名前をそのまま使わず、自分の実験に合わせて確定します。書き方と対象の対応は、[承認と実行の手順](https://github.com/shitada/foundry-distillation-lab/blob/main/docs/reference/cloud-execution.md#3-有料ネットワーク操作の承認)を参照してください。
...
送信前に記録を残し、結果が不明なら、まず元の要求の状態を照合します。結果が分からないまま、記録を削除したり実験名を変えたりして送り直さないでください。予算や件数の予約も、実際の請求額とは分けて残します。

### 条件を変更するときは、変更前の記録を残す

入力、指示文、ツール、設定を変えると、比較条件も変わる場合があります。変更した内容と理由を記録し、必要な承認を取り直して別の実験版として扱います。結果が悪かったことを理由に、期待する答えや合格基準だけを変えてはいけません。
...
- 操作ごとの承認、結果不明時の照合、所有する資源の後片付けの方針を説明できる。
...
4. **[本教材のクラウド実行手順](https://github.com/shitada/foundry-distillation-lab/blob/main/docs/reference/cloud-execution.md)／[安全な実行の方針](https://github.com/shitada/foundry-distillation-lab/blob/main/docs/reference/safety.md)**
   ...
   使用するライブラリ、承認、実行記録、停止・照合・後片付けの具体的な条件を確認できます。
