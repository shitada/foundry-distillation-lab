...
class RunnerTests(unittest.TestCase):
    ...
    def test_response_identity_is_retained_on_records_and_events(self):
        responses = model_responses()
        for index, response in enumerate(responses):
            response.update(response_model="resolved-model-version", response_id=f"response-{index}")
        record = run_case(self.case, "base", Mock(side_effect=responses), FakeSession)
        self.assertEqual(record["response_model"], "resolved-model-version")
        self.assertEqual(record["response_id"], f"response-{len(responses) - 1}")
        finishes = [event for event in record["events"] if event["event"] == "model_finish"]
        self.assertEqual([event["response_id"] for event in finishes],
                         [f"response-{index}" for index in range(len(responses))])
        case = {"case_id": "next", "messages": [{"role": "user", "content": "注文を確認してください。"}]}
        next_record = run_next_action(case, "base", Mock(return_value=responses[0]), fixture()["tools"])
        self.assertEqual(next_record["response_id"], "response-0")

    def test_incomplete_response_keeps_identity_and_unknown_usage(self):
        response = {"response_error": "incomplete_or_ambiguous_model_response",
                    "response_model": "resolved-version", "response_id": "incomplete"}
        record = run_case(self.case, "base", Mock(return_value=response), FakeSession)
        self.assertEqual(record["status"], "unknown")
...
class RecordingAndCliTests(unittest.TestCase):
    ...
    def recorded(self):
        return RecordedModel(self.transport, journal=self.journal, target="example-deployment")

    def test_target_must_be_explicit(self):
        with self.assertRaises(ValueError):
            RecordedModel(self.transport, journal=self.journal, target="")
        self.transport.assert_not_called()

    def test_target_and_write_ahead_before_transport(self):
        def invoke(payload):
            self.assertTrue((self.directory / "attempt-1.start.json").exists())
            self.assertEqual(payload["target"], "example-deployment")
            return {"message": {"content": "確認済み"}}
        self.transport.side_effect = invoke
        self.recorded()(self.payload)
    ...
    def test_journal_failure_stops_before_network(self):
        self.journal.start = Mock(side_effect=OSError)
    ...
    def test_old_send_flag_is_rejected(self):
        ...
        self.assertIn("unrecognized arguments: --send", result.stderr)

    def test_actual_journal_prevents_immutable_attempt_resend(self):
        from foundry_distillation_lab.safety import Journal
        journal = Journal(self.directory)
        journal.start("attempt-1", self.payload)
        recorded = RecordedModel(self.transport, journal=journal, target="example-deployment")
        with self.assertRaises(FileExistsError):
    ...
    def test_openai_boundary_is_lazy_and_mockable_with_retries_disabled(self):
        ...
        client.chat.completions.create.return_value.model_dump.return_value = {
            "model": "actual-model-version", "id": "chatcmpl-captured-id",
        ...
        self.assertEqual(response["response_model"], "actual-model-version")
        self.assertEqual(response["response_id"], "chatcmpl-captured-id")
        self.assertFalse(client.chat.completions.create.call_args.kwargs["store"])

    def test_openai_incomplete_response_keeps_raw_identity(self):
        transport = OpenAITransport(base_url="https://example.invalid/openai/v1/")
        ...
        response = transport({"target": "deployment", "messages": [], "tools": []})
